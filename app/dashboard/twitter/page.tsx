...
export default async function TwitterPage() {
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold" style={{ color: '#2E0510' }}>Twitter</h1>
        <p className="text-sm mt-0.5" style={{ color: '#906070' }}>Timeline dos personagens</p>
      </div>

      <TweetForm personagens={personagens ?? []} userId={user?.id ?? ''} />

      <div className="mt-6 space-y-0 divide-y" style={{ borderColor: 'rgba(128,0,32,0.10)' }}>
        {!tweets || tweets.length === 0 ? (
          <div className="py-12 text-center">
            <p className="text-sm" style={{ color: '#906070' }}>Nenhum tweet ainda. Escreva o primeiro!</p>
          </div>
        ) : (
          tweets.map(tweet => {
            ...
            return (
              <Link key={tweet.id} href={`/dashboard/twitter/${tweet.id}`} className="flex gap-3 py-4 px-1 hover:opacity-80 transition-opacity block">
                <div className="w-10 h-10 rounded-full overflow-hidden flex items-center justify-center shrink-0" style={{ background: 'rgba(128,0,32,0.08)' }}>
                  {personagem?.foto_url ? (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: '#2E0510' }}>{tweet.conteudo}</p>
                  <div className="flex items-center gap-5 mt-2">
                    <span className="flex items-center gap-1.5 text-xs" style={{ color: '#906070' }}>
