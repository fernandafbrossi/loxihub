  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#2E0510' }}>Instagram</h1>
          <p className="text-sm mt-0.5" style={{ color: '#906070' }}>Feed dos personagens</p>
        </div>
        <Link href="/dashboard/instagram/novo" className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90" style={{ background: '#800020', color: '#FAF0F2' }}>
      ...
      {!posts || posts.length === 0 ? (
        <div className="rounded-xl border p-12 text-center" style={{ background: 'rgba(255,255,255,0.60)', borderColor: 'rgba(128,0,32,0.10)' }}>
          <Camera size={40} className="mx-auto mb-4" style={{ color: '#906070' }} />
          <p className="font-medium mb-1" style={{ color: '#2E0510' }}>Nenhum post ainda</p>
          <Link href="/dashboard/instagram/novo" className="text-sm hover:underline" style={{ color: '#800020' }}>Criar primeiro post</Link>
        </div>
      ) : (
        <div className="space-y-6">
          {posts.map(post => {
            ...
            return (
              <div key={post.id} className="rounded-2xl border overflow-hidden" style={{ background: 'rgba(255,255,255,0.60)', borderColor: 'rgba(128,0,32,0.10)' }}>
                {/* Header */}
                <div className="flex items-center gap-3 p-4">
                  ...
                  <span className="font-semibold text-sm" style={{ color: '#2E0510' }}>
                ...
                <div className="px-4 pt-3 pb-1 flex items-center gap-4">
                  <button className="flex items-center gap-1.5 hover:opacity-70 transition-opacity" style={{ color: '#2E0510' }}>
                    <Heart size={22} />
                  </button>
                  <Link href={`/dashboard/instagram/${post.id}`} className="flex items-center gap-1.5 hover:opacity-70 transition-opacity" style={{ color: '#2E0510' }}>
                ...
                {/* Curtidas */}
                {curtidas > 0 && (
                  <p className="px-4 text-sm font-semibold" style={{ color: '#2E0510' }}>
                ...
                <Link href={`/dashboard/instagram/${post.id}`} className="block px-4 pb-4">
                  <p className="text-sm hover:underline" style={{ color: '#906070' }}>
