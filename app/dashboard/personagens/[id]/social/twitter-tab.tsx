...
import { Trash2, ImageIcon, X, Heart, MessageCircle, Repeat2, Pencil, Check, Send, Plus } from 'lucide-react'
...
export function TwitterTab({
  ...
  const posts = allPosts.filter(p =>
    ...
    (!p.conta_id && contas.indexOf(currentConta!) === 0 && (!p.tipo || p.tipo === 'twitter'))
  )
  ...
  async function criarConta() {
    ...
    const { data } = await supabase
      .from('contas_sociais')
      .insert({ personagem_id: personagemId, tipo: 'twitter', nome: novaNome.trim() })
      .select().single()
    if (data) {
  ...
  async function handlePost() {
    ...
    await supabase.from('posts_sociais').insert({
      ...
      curtidas: 0, comentarios: 0, retweets: 0,
      criado_por: user?.id,
    })
  ...
  return (
    <div>
      ...
      {!currentConta ? (
        ...
        <>
          {/* Bio + stats */}
          {(currentConta.bio || currentConta.seguindo > 0 || currentConta.seguidores > 0) && (
            <div className="mb-5 pb-5" style={{ borderBottom: '0.5px solid rgba(128,0,32,0.08)' }}>
              {currentConta.bio && <p className="text-sm leading-relaxed mb-3" style={{ color: '#2E0510' }}>{currentConta.bio}</p>}
              <div className="flex gap-5">
                <span className="text-xs" style={{ color: '#906070' }}>
                  <strong style={{ color: '#2E0510' }}>{currentConta.seguindo.toLocaleString('pt-BR')}</strong> seguindo
                </span>
                <span className="text-xs" style={{ color: '#906070' }}>
                  <strong style={{ color: '#2E0510' }}>{currentConta.seguidores.toLocaleString('pt-BR')}</strong> seguidores
                </span>
              </div>
            </div>
          )}
