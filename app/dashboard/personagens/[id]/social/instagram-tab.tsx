...
interface Personagem { id: string; nome: string; foto_url: string | null; username: string | null }

interface Post {
  id: string; conteudo: string; midia_url: string | null
  curtidas: number; comentarios: number; criado_por: string
  created_at: string; data_post: string | null
}
...
export function InstagramTab({ personagemId, personagem, posts: initialPosts, userId, bio, seguindo, seguidores, actingAs, todosPersonagens, curtidas: initCurtidas, comentarios: initComentarios }: Props) {
  ...
  const [engagements, setEngagements] = useState<Record<string, { curtidas: number; comentarios: number }>>(() => {
    ...
    for (const p of initialPosts) init[p.id] = { curtidas: p.curtidas ?? 0, comentarios: p.comentarios ?? 0 }
    return init
  })
  ...
  async function handlePost() {
    ...
    await supabase.from('posts_sociais').insert({
      ...
      tipo: 'instagram', data_post: new Date(dataPost).toISOString(),
      curtidas: 0, comentarios: 0, criado_por: user?.id,
    })
    setMidiaUrl(''); setLegenda(''); setShowForm(false); setLoading(false)
    ...
    <div>
      ...
      <div className="flex gap-6 mb-4 pb-4" style={{ borderBottom: '0.5px solid rgba(128,0,32,0.08)' }}>
        <div className="text-center">
          <p className="text-sm font-semibold" style={{ color: '#2E0510' }}>{initialPosts.length}</p>
          <p className="text-[10px]" style={{ color: '#906070' }}>publicações</p>
        </div>
        <div className="text-center">
          <p className="text-sm font-semibold" style={{ color: '#2E0510' }}>{seguidores.toLocaleString('pt-BR')}</p>
          <p className="text-[10px]" style={{ color: '#906070' }}>seguidores</p>
        </div>
        <div className="text-center">
          <p className="text-sm font-semibold" style={{ color: '#2E0510' }}>{seguindo.toLocaleString('pt-BR')}</p>
          <p className="text-[10px]" style={{ color: '#906070' }}>seguindo</p>
        </div>
      </div>
      {bio && <p className="text-sm leading-relaxed mb-4" style={{ color: '#2E0510' }}>{bio}</p>}
            ...
            return (
              <button key={post.id} onClick={() => { setSelected(post); setEditingCaption(false) }}
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3"
                  ...
                  <span className="flex items-center gap-1 text-xs font-semibold text-white"><Heart size={14} fill="white" /> {postCurtidas.length}</span>
                  <span className="flex items-center gap-1 text-xs font-semibold text-white"><MessageCircle size={14} fill="white" stroke="white" /> {postComentarios.length}</span>
                </div>
              </button>
            )
          })}
            ...
            <div className="w-full max-w-sm rounded-2xl overflow-hidden flex flex-col"
              ...
              <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0" style={{ borderBottom: '0.5px solid rgba(128,0,32,0.08)' }}>
                <div className="w-8 h-8 rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center text-xs font-semibold"
                  ...
                  {personagem.foto_url ? <img src={personagem.foto_url} alt="" className="w-full h-full object-cover" /> : personagem.nome[0].toUpperCase()}
                </div>
