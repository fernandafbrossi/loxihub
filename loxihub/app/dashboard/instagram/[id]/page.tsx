...
export default async function PostInstagramPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-6 max-w-md mx-auto">
      <Link href="/dashboard/instagram" className="inline-flex items-center gap-1.5 text-sm mb-5 hover:underline" style={{ color: 'var(--muted-foreground)' }}>
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
        <div className="flex items-center gap-3 p-4">
          <div className="w-9 h-9 rounded-full overflow-hidden flex items-center justify-center" style={{ background: 'var(--muted)' }}>
            {personagem?.foto_url ? <img src={personagem.foto_url} alt="" className="w-full h-full object-cover" /> : <Users size={16} style={{ color: 'var(--muted-foreground)' }} />}
          </div>
          <span className="font-semibold text-sm" style={{ color: 'var(--foreground)' }}>{personagem?.nome ?? 'Narrador'}</span>
        </div>
        ...
        <div className="p-4">
          <div className="flex items-center gap-3 mb-3">
            <Heart size={22} style={{ color: 'var(--foreground)' }} />
            <span className="text-sm font-semibold" style={{ color: 'var(--foreground)' }}>{curtidas ?? 0} curtidas</span>
          </div>
            ...
            {comentarios?.map(c => {
              ...
              return (
                <div key={c.id} className="flex items-start gap-2">
                  <div className="w-7 h-7 rounded-full overflow-hidden flex items-center justify-center shrink-0 mt-0.5" style={{ background: 'var(--muted)' }}>
                    {autor?.foto_url ? <img src={autor.foto_url} alt="" className="w-full h-full object-cover" /> : <Users size={12} style={{ color: 'var(--muted-foreground)' }} />}
                  </div>
